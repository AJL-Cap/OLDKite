...
const GamePage = () => {
  ...
  return (
    <div>
      ...
      <Container fluid>
        <Row>
          <Col>{games.map(game => <GameCard key={game.key} gameRef={game.ref} sessionRef={sessionRef} uid={user.uid} />)}</Col>
          <Col><RoomCodeForm sessionRef={sessionRef} uid={user.uid} /></Col>
        </Row>
      </Container>
    </div>
  );
